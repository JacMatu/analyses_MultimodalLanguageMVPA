...
%THIS SHOULD BE LOOPED ACROSS PAIRWISE CONDITIONS, BUT START SLOWLY? 
...
for iFWHM = 1:length(funcFWHM2Level)
    for iMod = 1:length(opt.cosmomvpa.modalities)
        for iCond = 1:length(opt.cosmomvpa.conditions)
            ...
            condition = opt.cosmomvpa.conditions{iCond};
...
condition = opt.cosmomvpa.conditions{3}; %just one condition now, but loop will be needed
...
for iFWHM = 1:length(funcFWHM2Level)
    for iMod = 1:length(opt.cosmomvpa.modalities)
        ...
        createContrastMaps(opt, subject_label, condition, modality, funcFWHM2Level(iFWHM))
