function winner_hdr = winner_take_all_cell(beta_images, mask_image, file_name)
    %
    % USAGE::
    %
    ...
    %if the input are cellstrings (due to uneven char length), convert them
    %to mat of structs 
    betas_hdr = cell2mat(spm_vol(beta_images));
    ...
    if ~isempty(mask_image)
        ...
        %winner(mask == 1) = max_indices(mask == 1); % BINARY MASK
        winner(mask > 0) = max_indices(mask > 0); % NON-BINARY MASK 
